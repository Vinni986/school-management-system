...
def principal_required(function=None, redirect_to="/"):
    def test_func(user):
        return user.is_active and (getattr(user, "is_principal", False) or user.is_superuser)
    def wrapper(request, *args, **kwargs):
        if test_func(request.user):
            return function(request, *args, **kwargs) if function else None
        return redirect(redirect_to)
...
def school_staff_required(function=None, redirect_to="/"):
    def test_func(user):
        return user.is_active and (getattr(user, "is_principal", False) or user.is_lecturer or user.is_superuser)
    def wrapper(request, *args, **kwargs):
        if test_func(request.user):
            return function(request, *args, **kwargs) if function else None
        return redirect(redirect_to)
